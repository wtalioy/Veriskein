...
mod tests {
    ...
    fn emits_ordered_event_without_drop() {
        ...
        let raw = build_exec_event_bytes(0, 1, 100, "bash", "/bin/bash", &["bash"]);
    ...
    fn synthesizes_gap_drop_event() {
        let mut collector = CollectorCore::new();
        let first = build_exec_event_bytes(1, 1, 101, "a", "/bin/a", &["a"]);
        let third = build_exec_event_bytes(1, 3, 103, "c", "/bin/c", &["c"]);
    ...
    fn synthesizes_reorder_event() {
        let mut collector = CollectorCore::new();
        let second = build_exec_event_bytes(2, 2, 102, "b", "/bin/b", &["b"]);
        let first = build_exec_event_bytes(2, 1, 101, "a", "/bin/a", &["a"]);
